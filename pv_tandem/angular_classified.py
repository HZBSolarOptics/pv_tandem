...
import numpy as np
import pandas as pd
from pv_tandem.bifacial_yield import IrradianceSimulator

class AngularAnalysis(IrradianceSimulator):
    def __init__(self, illumination_df, tilt_angle, module_spacing,
    albedo=0.3,
    module_length=1.96,
    module_height=0.5, 
    geo_kw_parameter={}, theta_bins="iso", filter_component=None):
        super().__init__(
            illumination_df,
            albedo,
            module_length,
        ...
        self.poa_irrad = self.simulate(spacing=module_spacing, tilt=tilt_angle, simple_results=False)
        
    def calculate_angular_dist_ground(self, matrix_name, illumination_name):
        eff_matrix = self.geo_instance.results[matrix_name]
    
        ground_illumination = self.geo_instance.results[illumination_name]
        ...
        mindex = pd.MultiIndex.from_product(
            [
                range(self.geo_instance.module_steps),
                range(self.geo_instance.angle_steps),
    ...
    def bin_theta_phi(self, irrad_component, module_position=5, agg_func=np.mean, theta_bins="iso"):
        df = irrad_component.loc[irrad_component["module_position"] == module_position]
    ...
    def process_sky_direct(self, theta_bins="iso"):
        ...
        df["theta"] = self.geo_instance.tmp["theta"].copy()
        df["phi"] = self.geo_instance.tmp["phi"].copy()
    ...
    def process_sky_diffuse(self, theta_bins="iso"):
        def calc_alpha_dist(alpha_array):
            ...
            dist_alpha = np.tile(
                dist_alpha, (self.geo_instance.module_steps, 1)
            )
        ...
        alpha_front = self.geo_instance.tmp["alpha_2_front"]
        ...
        alpha_back = -np.pi / 2 + self.geo_instance.tmp["epsilon_1_back"]
    ...
    def plot_polar_heatmap(self, filepath=None):
        ...
        front, back = self.calc_front_back()
        
        _plot_polar(front/1000)
        _plot_polar(back/1000)
...
if __name__ == "__main__":
    ...
    simulator = AngularAnalysis(illumination_df, tilt_angle=25, module_spacing=6)
