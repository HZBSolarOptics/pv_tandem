...
if __name__ == "__main__":
    ...
    pero_cell = OneDiodeModel(tcJsc=-0.001, tcVoc=-0.0013, R_shunt = 1000, R_series=4,
                             n=1.1, j0=2e-16)
    
    res = pero_cell.calc_iv_params(np.array([22]), np.array([25]))
    
    iv = pero_cell.calc_iv(np.array([22]), np.array([25]), j_arr=np.linspace(0, 45, 451))
    
    df_iv = pd.DataFrame({'voltage':iv[0], 'current':np.linspace(0, 45, 451)})
    ...
    exp_data['Intensity'] = (exp_data['Intensity'].astype(float)/10).round()*10
    ...
    V = pero_cell.calc_iv(Jsc=j_ph, cell_temp=T_arr, j_arr=np.linspace(0, 45, 451))
    ...
    cycle = ['C0', 'C1', 'C3']
    for i in range(3):
        ax.scatter(int_factors*100, V[:,i,0], marker='x', s=80, color=cycle[i])
