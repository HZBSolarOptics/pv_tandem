        ...
        if simple_results:
            ...
            results = pd.concat(
                [
                    ...
                    .T.groupby(level="module_position") 
                    .sum()
                    #.mean(axis=1),
                    .mean(),
...
class AngularAnalysis(IrradianceSimulator):
    ...
    def sum_binned_ground_data(self, theta_bins="iso", filter_component=None):
    ...
    def bin_theta_phi(
        self, irrad_component, agg_func=np.mean, theta_bins="iso"
    ):
        df = irrad_component.loc[
            irrad_component["module_position"] == self.module_position
        ]
        ...
        binned_data = (
            df.groupby(
                [
                    ...
                    pd.cut(
                        df.phi, np.linspace(-np.pi, np.pi, 37), labels=False
                    ),
                ]
            )["int"]
            .apply(agg_func)
    ...
    def process_sky_direct(self, theta_bins="iso"):
        ...
        df_back = df[
            ["theta", "phi", "back_sky_direct", "module_position"]
        ].copy()
        df_back["theta"] = np.pi - (df_back["theta"])
        df_back = df_back.rename(columns={"back_sky_direct": "int"})

        front = self.bin_theta_phi(
            df_front, agg_func=np.sum, theta_bins=theta_bins
        )
        ...
        if all(df_back.theta > np.pi / 2):
            ...
            back = self.bin_theta_phi(
                df_back, agg_func=np.sum, theta_bins=theta_bins
            )
    ...
    def process_sky_diffuse(self, theta_bins="iso"):
        ...
        df = pd.concat(
            [df_front, df_back], keys=["front", "back"], names=["side"]
        )
        ...
        binned_front = (
            self.bin_theta_phi(
                df.loc["front"], agg_func=np.sum, theta_bins=theta_bins
            )
            * self.dhi.sum()
        )
        binned_back = (
            self.bin_theta_phi(
                df.loc["back"], agg_func=np.sum, theta_bins=theta_bins
            )
            * self.dhi.sum()
        )
    ...
    def calc_front_back(self, theta_bins="iso", filter_component=None):
    ...
    def plot_polar_heatmap(
        self, theta_bins="iso", filepath_front=None, filepath_back=None
    ):
        ...
        def _plot_polar(df, filepath=None, prefix=None):
            ...
            plt.yticks(
                ...
                labels=map(
                    lambda x: str(int(x)) + "°", np.linspace(30, 60, 2)
                ),
            )
            ...
            cax.set_xlabel(
                r"Angle resolved radiant exposure" "\n" r"(kWh/m²/year)"
            )
